...
This plugin is inspired by GEdit's `External Tool` which enables you to compile and run your project in a efficient way. Each tool is represented by a single shell script defined by user. Tools can be used to execute Makefile or compile/run a single file with customizable compile flags and run options. 
